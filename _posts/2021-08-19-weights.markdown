...
There is one last subtlety about $ W $. It is not linked to the **information flow** so is $ X $. 
...
In [this article]({% post_url 2021-08-09-loss-function %}), we talked about "The derivative operator" and showed 
how it enabled us to compute the impact of any variable in $ model $ on the $ Loss $ function. This will prove 
useful now.

Because if we accept that the **weights** are a moving part of the $ model $, then we can **update** them 
...
We are now going to use the **learning flow** to compute the $ derivative $ functions of $ Loss $ according to 
the **weights**. 
...
We used the **learning flow** in order to compute the $ derivative $ functions of $ Loss $ according to these 
**weights**. In a side note, we remarked that we could merge both the **learning flow** and 
the $ derivative $ functions of $ Loss $ according to the **weights** in a same step: 
a generalized **backward pass**. From now on the **backward pass** will refer to this 
generalized **backward pass**.

The $ derivative $ functions of $ Loss $ according to these **weights** prove useful in order 
to update the values of the **weights** themselves which is the final goal of the **learning process**. 
We will recap the whole **learning process** in the [next article]({% post_url 2021-08-22-recapitulation %}).
...
Please note here that as we are doing exactly the same as in the "Run the backward pass for the learning flow" 
paragraph, we could merge the two steps "backward pass for the learning flow" and 
"backward pass for the weights" into one single **backward pass** that would systematically 
compute the $ derivative $ of the $ Loss $ function according to the variables declared in the 
current $ layer $: **input variable** $ X $ or **weights** variables $ W $. 
These all computations needing the **learning flow** from the previous $ layers $ 
(in the order of the **backward pass**), the logic stays the same.
