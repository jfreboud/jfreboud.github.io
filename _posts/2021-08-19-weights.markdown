...
1. If we pick one **data** in a **dataset** and compute a **forward pass** on it, we produce some results.
2. If we pick the exact same **data** in the **dataset** and compute a **forward pass** on it, we produce the 
exact same results. 

But as our goal is to learn from our failures we will use the **weights** in order to change that state: 
1. If we pick the previous **data** in the **dataset**, we compute a **forward pass** to produce some results
2. We **update** the **weights** values in order to learn
3. If we pick the exact same **data** in the **dataset** and compute a **forward pass** on it, the result we be 
different now.
...
We have just introduced the **weights** and said that the developer can change their values.
In this paragraph we elaborate on the special moment when the developer **updates** them: this is 
at the core of the **learning** process.

There is one last property that makes $ W $ so special: sometimes it will be a variable and sometimes not.
...
so that the modifications help the $ model $ to produce better results. 
...
What is important to keep in mind here is that the **weights** have a meta role. They are a junction between the 
...
the developer has to **update** them to make our $ model $ stronger. But we do not know how to **update** them.
The idea is to the the $ derivative $ operator.
...
running the **forward pass** of our $ model $ on some $ x $ **data** in the **dataset**. Then we observe the results 
produced by the $ model $ and the final $ loss $ value. Finally the magical move is that we can disturb slightly any 
variable of our $ model $ and still be able to "predict" what the new final error $ loss $ will be consequently 
to that change. 

We choose to apply our magical move to the $ W $ variable: we can disturb our **weights** slightly and still be able 
to "predict" the new final $ loss $ value. But in fact we do not really care about predicting this 
new value, what we really want is to minimize the $ loss $...
...
But if we add the same number negatively we "predict" that: $ f(x - h) < f(x) $ which is what we are looking for. 
...
the sign of the slope of the tangent, which is itself the the $ derivative $ of $ f $ 
according to $ X $ evaluated on $ x $ (according to our mathematical teacher). 

Finally we will remember that if we want to disturb slightly a variable of a function to minimize it, the small change 
has to be made in this direction: 
...
As we mentioned, we first have to compute the explicit formula and then it will be easy to evaluate it.
...
the **learning flow** for $ L^{k+1} $: $ \delta^{k+1} $. 
- So we have a link between $ L^{k} $ and $ L^{k+1} $. 
By definition of the **forward pass**: $ X^{k+1} = L^{k}(X^k) $. 
- And we have a link between $ L^{k+1} $ and $ Loss $ by the definition of the **learning flow** of $ L^{k+1} $. 

Thanks to the **chain rule** we are able to establish the link between $ W^{k} $ and $ Loss $. 
...
we can compute the **learning flow** for $ L^{k} $ (we already know that). Now we see we may as well 
compute the impact of its weights on the $ Loss $ : $ \delta w^{k} $. 

This means we may include that operation in the **backward pass** too: 
...
we must follow for our **weights**: 
...
The answer is really not long because as we mentioned several times, the "prediction" we talked about in 
the "The derivative operator" paragraph of [this article]({% post_url 2021-08-09-loss-function %}) is very localized 
around the point where we evaluate the $ derivative $ function. 
...
But finding a good value for this coefficient is up to the developer, a meta-algorithm of something else...
...
And with the previous notation we have:
...
they influence in later articles.
...
Note that $ L2 $ is the only $ layer $ that declares **weights** is this $ model $. 
...
according to these **weights** is in fact the direction to follow in order to **update** the **weights** so that 
we minimize the final $ loss $ value. 

We understood that this direction $ \delta w $ is computed during the **backward pass** along the **learning flow** and 
that it directly depends on the **learning flow** !

In the [next article]({% post_url 2021-08-22-recapitulation %}), we will recap the whole **learning process**.
