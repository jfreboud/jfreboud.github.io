...
Thus, we will now put these values inside the **neurons**. 
...
to each $ L^{k} $ **neuron**. The connexions being assured by the **weights** of $ L^{k} $: $ W^{k} $.
...
$$ 
\begin{align}
L^{k, 1}(X^{k}) &= W^{k, 1}_1 . X^{k, 1} + W^{k, 1}_2 . X^{k, 2} + W^{k, 1}_3 . X^{k, 3} + B^{k} \\
L^{k, 2}(X^{k}) &= W^{k, 2}_1 . X^{k, 1} + W^{k, 2}_2 . X^{k, 2} + W^{k, 2}_3 . X^{k, 3} + B^{k}
\end{align}
$$ 
...
- $ B^{k} $ is another **weight** variable, called the **bias**. 
...
Indeed, we have $ L^{k, 1} $ and $ L^{k, 2} $ which are the two output **neurons**.
Each of these variables is connected to $ X^{k, 1} $, $ X^{k, 2} $ and $ X^{k, 3} $ which are the 3 input 
...
Note that we have one more **weight**: $ B^{k} $ that is used the same in $ L^{k, 1} $ and in $ L^{k, 2} $. 
...
The interesting variable is $ X^{k}_1 $. In the different diagrams it corresponds to $ o^{k-1}_1 $, 
...
$ o^{k}_1 $ is linked to $ o^{k-1}_1 $ and that $ o^{k}_2 $ is also linked to $ o^{k-1}_1 $.
...
$$ 
\delta^{k}_1 = \delta^{k+1}_1 * \frac{\partial L^{k}_1}{X^{k}_1}(o^k_1)  + 
\delta^{k+1}_2 * \frac{\partial L^{k}_2}{X^{k}_1}(o^k_1)
$$
...
\frac{\partial (W^{k, 1}_1 . X^{k}_1 + W^{k, 1}_2 . X^{k}_2 + W^{k, 1}_3 . X^{k}_3 + B^{k})}{\partial X^{k}_1} \\
                                          &= W^{k, 1}_1 
\end{align}
$$
...
$$ 
\delta^{k, 2} = \frac{\partial Loss}{\partial X^{k, 2}}(o^{k-1}_2)
$$

The interesting variable is $ X^{k, 2} $. In the different diagrams it corresponds to $ o^{k-1}_2 $, 
its value during the current **backward pass**. As in the previous paragraph, 
every output of $ L^{k} $ uses $ X^{k, 2} $.
Indeed we can look at the last two diagrams [here](#linear-structure1) to see that 
$ o^{k}_1 $ is linked to $ o^{k-1}_2 $ and that $ o^{k}_2 $ is also linked to $ o^{k-1}_2 $.

We are now able to build the **paths** of impacts from $ X^{k+1, 2} $ to the $ Loss $ function. 

![Linear](/_assets/images/layers/Linear7.png)

- $ X^{k, 2} $ impacts $ L^{k, 1} $ which impacts the $ Loss $ function 
- $ X^{k, 2} $ impacts $ L^{k, 2} $ which impacts the $ Loss $ function 

We add these 2 impacts, using the **chain rule**, to obtain: 

$$ 
\delta^{k, 2} = \delta^{k+1, 1} * \frac{\partial L^{k, 1}}{X^{k, 2}}(o^{k-1}_2)  + 
\delta^{k+1, 2} * \frac{\partial L^{k, 2}}{X^{k, 2}}(o^{k-1}_2)
$$
...
$$ 
\begin{align}
\frac{\partial L^{k, 1}}{\partial X^{k, 2}} &= 
\frac{\partial (W^{k, 1}_1 . X^{k, 1} + W^{k, 1}_2 . X^{k, 2} + W^{k, 1}_3 . X^{k, 3} + B^{k})}{\partial X^{k, 2}} \\
                                            &= W^{k, 1}_2 
\end{align}
$$
...
$$ 
\frac{\partial L^{k, 1}}{X^{k, 2}}(o^{k-1}_2) = w^{k, 1}_2 
$$

We do the same to obtain: 

$$ 
\frac{\partial L^{k, 2}}{X^{k, 2}}(o^{k-1}_2) = w^{k, 2}_2
$$

We finally assemble these two results:

$$ 
\boxed{\delta^{k, 2} = \delta^{k+1, 1} * w^{k, 1}_2 + \delta^{k+1, 2} * w^{k, 2}_2}
$$

### Computing $ \delta^{k, 3} $ 

$$ 
\delta^{k, 3} = \frac{\partial Loss}{\partial X^{k, 3}}(o^{k-1}_3)
$$

The interesting variable is $ X^{k, 3} $. In the different diagrams it corresponds to $ o^{k-1}_3 $, 
its value during the current **backward pass**. As in the previous paragraph, 
every output of $ L^{k+1} $ uses $ X^{k, 3} $.
Indeed we can look at the last two diagrams [here](#linear-structure1) to see that 
$ o^{k}_1 $ is linked to $ o^{k-1}_3 $ and that $ o^{k}_2 $ is also linked to $ o^{k-1}_3 $.

We are now able to build the **paths** of impacts from $ X^{k, 3} $ to the $ Loss $ function. 

![Linear](/_assets/images/layers/Linear8.png)

- $ X^{k, 3} $ impacts $ L^{k, 1} $ which impacts the $ Loss $ function 
- $ X^{k, 3} $ impacts $ L^{k, 2} $ which impacts the $ Loss $ function 

We add these 2 impacts, using the **chain rule**, to obtain: 

$$ 
\delta^{k, 3} = \delta^{k+1, 1} * \frac{\partial L^{k, 1}}{X^{k, 3}}(o^{k-1}_3)  + 
\delta^{k+1, 2} * \frac{\partial L^{k, 2}}{X^{k, 3}}(o^{k-1}_3)
$$
...
$$ 
\begin{align}
\frac{\partial L^{k, 1}}{\partial X^{k, 3}} &= 
\frac{\partial (W^{k, 1}_1 . X^{k, 1} + W^{k, 1}_2 . X^{k, 2} + W^{k, 1}_3 . X^{k, 3} + B^{k})}{\partial X^{k, 3}} \\
                                            &= W^{k, 1}_3 
\end{align}
$$
...
$$ 
\frac{\partial L^{k, 1}}{X^{k, 3}}(o^{k-1}_3) = w^{k, 1}_3 
$$

We do the same to obtain: 

$$ 
\frac{\partial L^{k, 2}}{X^{k, 3}}(o^{k-1}_3) = w^{k, 2}_3
$$

We finally assemble these two results:

$$ 
\boxed{\delta^{k, 3} = \delta^{k+1, 1} * w^{k, 1}_3 + \delta^{k+1, 2} * w^{k, 2}_3}
$$
...
There is one last **weight** we have to **update** during the **learning phase**: $ B^{k} $, the **bias**. 
Thus, we will have to compute $ \delta b^{k} $ too.
...
- $ W^{k, 1}_1 $ impacts $ L^{k, 1} $ which impacts the $ Loss $ function  

We have only 1 impact, using the **chain rule**, we obtain: 

$$ 
\delta w^{k, 1}_1 = \delta^{k+1, 1} * \frac{\partial L^{k, 1}}{W^{k, 1}_1}(o^{k-1}_1)
$$
...
$$ 
\begin{align}
\frac{\partial L^{k, 1}}{\partial W^{k, 1}_1} &= 
\frac{\partial (W^{k, 1}_1 . X^{k, 1} + W^{k, 1}_2 . X^{k, 2} + W^{k, 1}_3 . X^{k, 3} + B^{k})}{\partial W^{k, 1}_1} \\
                                              &= X^{k, 1}
\end{align}
$$
...
$$ 
\frac{\partial L^{k, 1}}{W^{k, 1}_1}(o^{k-1}_1) = o^{k-1}_1
$$

We finally use this result in the first formula:

$$ 
\boxed{\delta w^{k, 1}_1 = \delta^{k+1, 1} * o^{k-1}_1}
$$
...
$$ 
\boxed{\delta w^{k, 1}_2 = \delta^{k+1, 1} * o^{k-1}_2}
$$

$$ 
\boxed{\delta w^{k, 1}_3 = \delta^{k+1, 1} * o^{k-1}_3}
$$
...
- $ W^{k, 2}_1 $ impacts $ L^{k, 2} $ which impacts the $ Loss $ function  

We have only 1 impact, using the **chain rule**, we obtain: 

$$ 
\delta w^{k, 2}_1 = \delta^{k+1, 2} * \frac{\partial L^{k, 2}}{W^{k, 2}_1}(o^{k-1}_1)
$$
...
$$ 
\begin{align}
\frac{\partial L^{k, 2}}{\partial W^{k, 2}_1} &= 
\frac{\partial (W^{k, 2}_1 . X^{k, 1} + W^{k, 2}_2 . X^{k, 2} + W^{k, 2}_3 . X^{k, 3} + B^{k})}{\partial W^{k, 2}_1} \\
                                              &= X^{k, 1}
\end{align}
$$
...
$$ 
\frac{\partial L^{k, 2}}{W^{k, 2}_1}(o^{k-1}_1) = o^{k-1}_1
$$

We finally use this result in the first formula:

$$ 
\boxed{\delta w^{k, 2}_1 = \delta^{k+1, 2} * o^{k-1}_1}
$$
...
$$ 
\boxed{\delta w^{k, 2}_2 = \delta^{k+1, 2} * o^{k-1}_2}
$$

$$ 
\boxed{\delta w^{k, 2}_3 = \delta^{k+1, 2} * o^{k-1}_3}
$$

### Computing $ \delta b^{k} $ 

$$ 
\delta b^{k} = \frac{\partial Loss}{\partial B^{k, 1}}(o^{k-1})
$$

The interesting variable is $ B^{k} $. Every output of $ L^{k} $ uses $ B^{k} $. 
This is clear when we take a look at the first two equations of the [forward pass](#forward-pass) paragraph. 

We are now able to build the **paths** of impacts from $ B^{k} $ to the $ Loss $ function. 

- $ B^{k} $ impacts $ L^{k, 1} $ which impacts the $ Loss $ function  
- $ B^{k} $ impacts $ L^{k, 2} $ which impacts the $ Loss $ function  

We add these 2 impacts, using the **chain rule**, to obtain: 

$$ 
\delta b^{k} = \delta^{k+1, 1} * \frac{\partial L^{k, 1}}{B^{k}}(o^{k-1}) + 
\delta^{k+1, 2} * \frac{\partial L^{k, 2}}{B^{k}}(o^{k-1})
$$
...
$$ 
\begin{align}
\frac{\partial L^{k, 1}}{\partial B^{k}} &= 
\frac{\partial (W^{k, 1}_1 . X^{k, 1} + W^{k, 1}_2 . X^{k, 2} + W^{k, 1}_3 . X^{k, 3} + B^{k})}{\partial B^{k}} \\
                                         &= 1
\end{align}
$$
...
$$ 
\frac{\partial L^{k, 1}}{B^{k}}(o^{k-1}) = 1
$$

We do the same to obtain: 

$$ 
\frac{\partial L^{k, 2}}{B^{k}}(o^{k-1}) = 1
$$

We finally assemble these two results:

$$ 
\boxed{\delta b^{k} = \delta^{k+1, 1} + \delta^{k+1, 2}}
$$
...
$$
\delta^{k, 1} = \delta^{k+1, 1} * w^{k, 1}_1 + \delta^{k+1, 2} * w^{k, 2}_1
$$

$$ 
\delta^{k, 2} = \delta^{k+1, 1} * w^{k, 1}_2 + \delta^{k+1, 2} * w^{k, 2}_2
$$

$$ 
\delta^{k, 3} = \delta^{k+1, 1} * w^{k, 1}_3 + \delta^{k+1, 2} * w^{k, 2}_3
$$
...
$$ 
\delta w^{k, 1}_1 = \delta^{k+1, 1} * o^{k-1}_1
$$

$$ 
\delta w^{k, 1}_2 = \delta^{k+1, 1} * o^{k-1}_2
$$

$$ 
\delta w^{k, 1}_3 = \delta^{k+1, 1} * o^{k-1}_3
$$
